...
| `product/theraneck-360.mp4` | **The owner's 360 turntable render** of TheraNECK+ (`~/Downloads/TheraNECK_360_5.mp4`, 1080x1080, 96 frames at 30fps). Re-encoded to 900x900 CRF 30 **all-intra** — `keyint=1`, so all 96 frames are keyframes — which is the whole point: `Product360` seeks to an arbitrary frame on every pointer move, and the source carried exactly ONE keyframe for the clip, so dragging backwards would have decoded from the start each time. 0.36 MB, SSIM 0.993 against the source. |
| `product/theraneck-360-poster.jpg` | Frame 0 of that clip, the `<video poster>`, so the panel is never an empty box. |
| | The owner also sent `TheraNECK_1.glb` (1.57 MB, 46 meshes). It is **not** in the repo: a true 3D viewer needs a WebGL runtime and a CDN component, and its appearance depends on lighting we would have to author — none of which can be seen in this environment, since the browser pane does not paint WebGL. The file is still in `~/Downloads` if a 3D/AR viewer is wanted later. |
