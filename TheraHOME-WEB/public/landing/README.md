...
| `cine/00.mp4`, `cine/03.mp4` | **The owner's own footage**, 1248x704 (which is exactly the design's `sourceAspect`). `00.mp4` is `~/Downloads/Cinematic Architectural Video.mp4` (8.04s, the approach with the doors opening); `03.mp4` is `~/Downloads/3.mp4` (5.04s, the foyer walk). Copied unchanged — re-encoding would cost frame accuracy, and the scroll scrubs these frame by frame. 4.2 MB together; the script fetches each as a blob so seeking is instant once loaded. |
...
| `cine/00-poster.jpg` | Frame 0 of `00.mp4`, pulled with `qlmanage -t`. Backs the loading card so the opening shot is on screen while the video downloads, instead of the design's flat dark square. Same frame, so nothing jumps when the video takes over. |
