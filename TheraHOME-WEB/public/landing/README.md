...
| `cine/00.mp4`, `cine/03.mp4` | **The owner's own footage**, 1248x704 (exactly the design's `sourceAspect`). `00.mp4` is `~/Downloads/Cinematic Architectural Video.mp4` (8.04s, the approach with the doors opening); `03.mp4` is `~/Downloads/3.mp4` (5.04s, the foyer walk). **Re-encoded 2026-09-18** — the originals are HEVC with 2 and 1 keyframes respectively, which is both a compatibility problem (HEVC in MP4 needs hardware decode; plenty of Chrome installs cannot play it) and a scrubbing problem (a seek decodes from the nearest keyframe, so a whole-clip GOP makes backward scrubbing lurch). Now H.264 Main, CRF 30, `preset veryslow`, `tune film`, `keyint=12:min-keyint=12:scenecut=0` — a keyframe every 0.47s, 17 and 11 of them, audio stripped, `+faststart`. 2.65 MB together, down from 3.99 MB, SSIM 0.960 against the source. Backward seeks measured at 6.3 ms average in the browser. |
