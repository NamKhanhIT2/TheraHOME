...
// COMPANY LIMITED" (no bracketed placeholders remain — "[Tên bạn]" in the
// subscription-cancel step is iOS Settings UI wording, not a placeholder).
// The wording itself is still AI-drafted, not counsel-reviewed — worth a
// legal pass before App Store / Play Store submission. See CLAUDE.md.
//
...
// (`legalContentMs`) versions below are AI translations of that Vietnamese
// original and have NOT been reviewed by counsel — they need a legal review
// before any release that presents them as binding. Use `getLegalDoc()` to
// pick a document by language (falls back to Vietnamese).
//
// KEEP IN SYNC: the public web copy of this file lives at
// TheraHOME-WEB/src/lib/appLegalContent.ts — any change here (wording,
// structure, or new language) must be mirrored there, and vice versa.
