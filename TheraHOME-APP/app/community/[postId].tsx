...
export default function PostDetailScreen() {
  ...
  return <ScreenContainer><KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
    ...
    <View style={[styles.composer, { borderTopColor: theme.colors.divider, backgroundColor: theme.colors.bgApp }]}>{!editing ? <Pressable accessibilityRole="button" accessibilityLabel="Chọn ảnh hoặc video" onPress={pickImage} hitSlop={8} style={styles.composerTool}><Icon name="image" size={25} color={theme.colors.primary} /></Pressable> : null}<TextInput ref={inputRef} value={text} onChangeText={setText} onFocus={() => setShowEmoji(false)} multiline maxLength={2000} placeholder={editing ? t('editCommentPlaceholder') : replyTo ? t('replyTo', { name: replyTo.name }) : t('writeComment')} accessibilityLabel="Nội dung bình luận" placeholderTextColor={theme.colors.textMuted} style={[styles.input, { backgroundColor: theme.colors.bgCardAlt, borderColor: theme.colors.borderInput, borderRadius: theme.radius.full, color: theme.colors.textPrimary }]} /><Pressable accessibilityRole="button" accessibilityLabel="Chọn biểu tượng cảm xúc" onPress={() => setShowEmoji((v) => !v)} hitSlop={8} style={styles.composerTool}><Icon name="smile" size={25} color={theme.colors.primary} /></Pressable><Pressable accessibilityRole="button" accessibilityLabel={editing ? 'Lưu bình luận' : 'Gửi bình luận'} onPress={submit} disabled={(!text.trim() && !imageUri) || pending} style={[styles.send, { backgroundColor: theme.colors.primary, opacity: (!text.trim() && !imageUri) || pending ? 0.45 : 1 }]}>{pending ? <ActivityIndicator size="small" color="#fff" /> : <Icon name="send" size={16} color="#fff" />}</Pressable></View>
