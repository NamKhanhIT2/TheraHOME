...
export default function PostDetailScreen() {
  ...
  // Android draws modals under the navigation bar (edge-to-edge); keep the
  // report sheet above it.
  ...
  // every background refetch, and this screen refetches whenever anyone
  // reacts to any comment anywhere — so keeping it in the deps yanked the
  // reader's scroll back to the linked comment and re-flashed the highlight
  // at random moments while they were reading.
  const scrolledToTarget = useRef<string | null>(null);
  useEffect(() => {
    if (!targetId || !comments.length || scrolledToTarget.current === targetId) return;
    ...
    scrolledToTarget.current = targetId;
  ...
  if (postQuery.isError) return <ScreenContainer><BackBar onBack={() => router.back()} title={t('postDetails')} /><View style={styles.center}><Text style={[theme.type.bodyStrong, { color: theme.colors.textPrimary, textAlign: 'center' }]}>{t('homeLoadErrorTitle')}</Text><Text style={[theme.type.caption, { color: theme.colors.textSecondary, textAlign: 'center', marginTop: 6 }]}>{t('checkNetworkRetry')}</Text><Pressable onPress={() => postQuery.refetch()} style={{ marginTop: 14 }}><Text style={[theme.type.bodyStrong, { color: theme.colors.primary }]}>{t('retry')}</Text></Pressable></View></ScreenContainer>;
  ...
  function remove(comment: CommentRow) {
    closeCommentMenu();
    Alert.alert(t('deleteCommentConfirmTitle'), t('deleteCommentConfirmBody'), [{ text: t('cancel'), style: 'cancel' }, { text: t('delete'), style: 'destructive', onPress: () => deleteComment.mutate({ commentId: comment.id, postId: post!.id }, { onError: (cause) => setError(friendlyCommunityError(cause)) }) }]);
  }
    ...
    return <Reanimated.View key={comment.id} entering={fadeUpEntering(nested ? 0 : staggerDelay(index, 30, 8), 6, 200)} onLayout={(e) => { if (!nested) positions.current[comment.id] = e.nativeEvent.layout.y; }} style={[nested ? [styles.nested, { borderLeftColor: theme.colors.borderLight }] : styles.commentBlock, highlight === comment.id || commentReactionPicker?.comment.id === comment.id ? { backgroundColor: theme.colors.primaryTint05, borderRadius: theme.radius.md } : null]}>
      <View style={styles.commentRow}>
        ...
        <ReactionButton
          ...
          onLongPress={({ pageX, pageY }) => setCommentReactionPicker({ comment, pageX, pageY, hovered: null })}
          onLongPressMove={(point) => setCommentReactionPicker((active) => active?.comment.id === comment.id ? { ...active, hovered: pickerReactionAt(point, getReactionTrayFrame(active, windowWidth)) } : active)}
          onLongPressRelease={() => {
  ...
  return <ScreenContainer><KeyboardAvoidingView style={{ flex: 1 }} behavior="padding">
    ...
    <ScrollView ref={scrollRef} contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled" onTouchStart={() => setShowEmoji(false)} onScrollBeginDrag={() => setShowEmoji(false)} refreshControl={<RefreshControl refreshing={manualRefreshing} onRefresh={async () => { setManualRefreshing(true); try { await Promise.all([postQuery.refetch(), commentsQuery.refetch()]); } finally { setManualRefreshing(false); } }} tintColor={theme.colors.primary} />}>
      <View style={styles.post}>
        ...
        <PostActionBar
          ...
          onReactionLongPress={({ pageX, pageY }) => setPostReactionPicker({ pageX, pageY, hovered: null })}
          onReactionLongPressMove={(point) => setPostReactionPicker((active) => active ? { ...active, hovered: pickerReactionAt(point, getReactionTrayFrame(active, windowWidth)) } : active)}
      ...
      <View style={[styles.commentsTitle, { borderTopColor: theme.colors.divider }]}><Text style={[theme.type.bodyStrong, { color: theme.colors.textPrimary }]}>{t('commentsTitle')}</Text></View>
    ...
    <View style={[styles.composer, { borderTopColor: theme.colors.divider, backgroundColor: theme.colors.bgApp }]}>{!editing ? <Pressable accessibilityRole="button" accessibilityLabel={t('a11yPickMedia')} onPress={pickImage} hitSlop={8} style={styles.composerTool}><Icon name="image" size={25} color={theme.colors.primary} /></Pressable> : null}<TextInput ref={inputRef} value={text} onChangeText={setText} onFocus={() => setShowEmoji(false)} multiline maxLength={2000} placeholder={editing ? t('editCommentPlaceholder') : replyTo ? t('replyTo', { name: replyTo.name }) : t('writeComment')} accessibilityLabel={t('a11yCommentInput')} placeholderTextColor={theme.colors.textMuted} style={[styles.input, { backgroundColor: theme.colors.bgCardAlt, borderColor: theme.colors.borderInput, borderRadius: theme.radius.full, color: theme.colors.textPrimary }]} /><Pressable accessibilityRole="button" accessibilityLabel={t('a11yPickEmoji')} onPress={() => setShowEmoji((v) => !v)} hitSlop={8} style={styles.composerTool}><Icon name="smile" size={25} color={theme.colors.primary} /></Pressable><Pressable accessibilityRole="button" accessibilityLabel={editing ? t('a11ySaveComment') : t('a11ySendComment')} onPress={submit} disabled={(!text.trim() && !imageUri) || pending} style={[styles.send, { backgroundColor: theme.colors.primary, opacity: (!text.trim() && !imageUri) || pending ? 0.45 : 1 }]}>{pending ? <ActivityIndicator size="small" color="#fff" /> : <Icon name="send" size={16} color="#fff" />}</Pressable></View>
  </KeyboardAvoidingView>
  <Modal visible={!!postMenu} transparent animationType="fade" onRequestClose={() => setPostMenu(null)}><Pressable style={styles.commentOverlay} onPress={() => setPostMenu(null)}>{postMenu ? <Pressable onPress={() => undefined} style={[styles.floatingPostMenu, { top: Math.min(Math.max(88, postMenu.pageY - 8), windowHeight - 280), left: Math.max(12, Math.min(postMenu.pageX - 190, windowWidth - 232)), backgroundColor: theme.colors.bgCard, borderColor: theme.colors.borderInput, borderRadius: theme.radius.lg }]}><Pressable onPress={() => { toggleSave.mutate({ postId: post.id, saved }); setPostMenu(null); }} style={styles.item}><Icon name="bookmark" size={18} color={theme.colors.textSecondary} /><Text style={[theme.type.body, { color: theme.colors.textPrimary }]}>{saved ? t('unsave') : t('savePost')}</Text></Pressable>{!ownPost ? <><Pressable onPress={() => { hidePost.mutate(post.id, { onSuccess: () => router.back(), onError: (cause) => setError(friendlyCommunityError(cause)) }); setPostMenu(null); }} style={styles.item}><Icon name="eye" size={18} color={theme.colors.textSecondary} /><Text style={[theme.type.body, { color: theme.colors.textPrimary }]}>{t('hidePost')}</Text></Pressable><Pressable onPress={() => { setPostMenu(null); setReportTarget({ id: post.id, type: 'post' }); }} style={styles.item}><Icon name="flag" size={18} color={theme.colors.error} /><Text style={[theme.type.body, { color: theme.colors.error }]}>{t('reportPost')}</Text></Pressable><Pressable onPress={() => { setPostMenu(null); if (!post.authorId) return; Alert.alert(t('blockUserConfirmTitle'), t('blockUserConfirmBody', { name: post.authorName }), [{ text: t('cancel'), style: 'cancel' }, { text: t('blockUser'), style: 'destructive', onPress: () => blockUser.mutate(post.authorId!, { onSuccess: () => router.back(), onError: (cause) => setError(friendlyCommunityError(cause)) }) }]); }} style={styles.item}><Icon name="user-x" size={18} color={theme.colors.error} /><Text style={[theme.type.body, { color: theme.colors.error }]}>{t('blockUser')}</Text></Pressable></> : <><Pressable onPress={() => { setPostDraft(post.text); setPostEditing(true); setPostMenu(null); }} style={styles.item}><Icon name="pencil" size={18} color={theme.colors.textSecondary} /><Text style={[theme.type.body, { color: theme.colors.textPrimary }]}>{t('editPost')}</Text></Pressable><Pressable onPress={() => { setPostMenu(null); Alert.alert(t('deletePost'), t('deletePostConfirmBody'), [{ text: t('cancel'), style: 'cancel' }, { text: t('delete'), style: 'destructive', onPress: () => deletePost.mutate(post.id, { onSuccess: () => router.back(), onError: (cause) => setError(friendlyCommunityError(cause)) }) }]); }} style={styles.item}><Icon name="trash-2" size={18} color={theme.colors.error} /><Text style={[theme.type.body, { color: theme.colors.error }]}>{t('deletePost')}</Text></Pressable></>}</Pressable> : null}</Pressable></Modal>
...
const styles = StyleSheet.create({
  ...
  commentsTitle: { borderTopWidth: StyleSheet.hairlineWidth, paddingTop: 14, marginTop: 4, marginBottom: 10 }, commentBlock: { marginBottom: 16 }, nested: { marginTop: 12, borderLeftWidth: 2, paddingLeft: 10 }, commentRow: { flexDirection: 'row', gap: 9, alignItems: 'flex-start' }, bubble: { alignSelf: 'stretch', paddingTop: 2, paddingRight: 2, maxWidth: '100%' }, commentNameLine: { flexDirection: 'row', alignItems: 'baseline', gap: 5 }, meta: { flexDirection: 'row', gap: 12, alignItems: 'center', marginTop: 5, marginLeft: 1 }, thread: { marginTop: 7, marginLeft: 43, gap: 4 }, load: { alignSelf: 'center', borderWidth: 1, borderRadius: 999, paddingHorizontal: 16, paddingVertical: 9, marginTop: 4 },
  banner: { paddingHorizontal: 20, paddingVertical: 7, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, imagePreview: { paddingHorizontal: 20, paddingTop: 8, borderTopWidth: 1, height: 76 }, preview: { width: 62, height: 62 }, videoPreview: { alignItems: 'center', justifyContent: 'center' }, remove: { position: 'absolute', left: 72, top: 5, width: 20, height: 20, borderRadius: 10, alignItems: 'center', justifyContent: 'center' }, emojiBar: { flexDirection: 'row', justifyContent: 'space-around', paddingVertical: 9, borderTopWidth: 1 }, emoji: { fontSize: 24 }, error: { paddingHorizontal: 20, paddingTop: 4 }, composer: { flexDirection: 'row', alignItems: 'flex-end', gap: 4, paddingHorizontal: 12, paddingVertical: 8, borderTopWidth: 1 }, composerTool: { width: 40, height: 40, alignItems: 'center', justifyContent: 'center' }, input: { flex: 1, minHeight: 40, maxHeight: 104, borderWidth: 1, paddingHorizontal: 13, paddingVertical: 9, fontSize: 14, textAlignVertical: 'center' }, send: { width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center' },
  overlay: { flex: 1, justifyContent: 'flex-end', padding: 16 }, sheet: { padding: 12, gap: 2 }, item: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 9, paddingVertical: 13 }, selected: { padding: 12, marginBottom: 8 }, report: { paddingVertical: 13, borderTopWidth: StyleSheet.hairlineWidth },
