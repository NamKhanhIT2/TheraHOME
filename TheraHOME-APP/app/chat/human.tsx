...
import { markSpecialistMessagesRead, toggleChatReaction, useChatMessages, useChatThread, useSendChatMessage, useSpecialistPresence, type ChatMessageRow } from '@/hooks/useChat';
    ...
    return (
      <View style={styles.messageRow}>
        <View style={[styles.messageWrap, { alignSelf: own ? 'flex-end' : 'flex-start' }]}> 
          ...
          {own && (message.id === latestOwnId || pending) ? <Text style={[styles.deliveryStatus, { color: theme.colors.textMuted }]}>{pending ? t('sending') : message.readAt ? t('seen') : t('delivered')}{message.editedAt ? ` · ${t('edited')}` : ''}</Text> : null}
        </View>
