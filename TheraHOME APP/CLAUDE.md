...
editing + real account deletion — all on real per-user data. See Phase
roadmap below for what's left, which at this point is exclusively external
credentials/decisions the user still has to provide, not app code.
...
backend gets built against — see Chat below.
...
- **Auth**: three methods, all funneled through the same post-auth gate in
  ...
  method (see "Three login methods, one gate" below).
  - **Google**: `src/lib/googleAuth.ts`, the standard Supabase+Expo
    ...
    manual setup is done — see below.**
  - **Apple**: `src/lib/appleAuth.ts`, native `expo-apple-authentication`
    ...
    Developer/Supabase dashboard setup — see Manual setup below.
  - **TheraHOME account**: `app/(onboarding)/thera-login.tsx`, plain
    ...
    OAuth, no purchase involved. See "TheraHOME-issued accounts" below.
...
- **Chat presence**: `useSpecialistPresence()` (`src/hooks/useChat.ts`) uses
  ...
  channel name. No such client exists yet (see Chat below), so it correctly
  shows offline rather than a hardcoded "online" badge.
...
- **Images**: `expo-image-picker` uploads to public Storage buckets under a
  ...
  `avatars` (profile photos, Phase 6). See Supabase schema below.
- **Push/local notifications**: `expo-notifications`, wired in
  ...
  Firebase credentials under Manual setup below, same as EAS itself.
...
app/                        Expo Router routes (file-based)
  ...
  (onboarding)/               welcome → questions → consent → login → activate
                              (login/activate call real Supabase — see below)
...
No native build step is required for any of this — everything, including
`expo-image-picker` and local `expo-notifications` reminders, runs in plain
Expo Go (both just need their `app.json` plugin entries, already present).
Only remote push delivery needs an EAS dev client — see Remaining external
items above.
